...
! For a description of Craig-Bamptom analyses, see Appendix D to the MYSTRAN User's Referance Manual
      ...
      CHARACTER(LEN(ALLOW_OU4_MYSTRAN_NAMES)):: MAT_NAME     ! Matrix name
      CHARACTER(LEN(ALLOW_OU4_MYSTRAN_NAMES)):: MAT_NAME_OUT ! Matrix name written to OUTPUT4 file
      ...
      INTEGER(LONG)                   :: FORM                ! NASTRAN FORM of a matrix
      ...
      INTEGER(LONG)                   :: UNT                 ! File unit number where to write OU4 matrix
...
! Read KRL stiffness matrix
      ...
      IF (NTERM_KRL > 0) THEN                              ! Allocate and read KGG stiffness matrix
         ...
         CALL READ_MATRIX_1 ( LINK2K, L2K, OPND, CLOSE_IT, L2KSTAT, L2K_MSG, 'KRL', NTERM_KRL, NTERM_READ, NDOFR                   &
                            , I_KRL, J_KRL, KRL)
      ENDIF
...
! Read KRR stiffness matrix
      ...
      IF (NTERM_KRR > 0) THEN                              ! Allocate and read KGG stiffness matrix
         ...
         WRITE(SC1,6092) LINKNO,MODNAM,HOUR,MINUTE,SEC,SFRAC
         CALL READ_MATRIX_1 ( LINK2L, L2L, OPND, CLOSE_IT, L2LSTAT, L2L_MSG, 'KRR', NTERM_KRR, NTERM_READ, NDOFR                   &
                            , I_KRR, J_KRR, KRR)
      ENDIF
...
! Read MRL mass matrix
      ...
      IF (NTERM_MRL > 0) THEN                              ! Allocate and read KGG stiffness matrix
         ...
         WRITE(SC1,6092) LINKNO,MODNAM,HOUR,MINUTE,SEC,SFRAC
         CALL READ_MATRIX_1 ( LINK2M, L2M, OPND, CLOSE_IT, L2MSTAT, L2M_MSG, 'MRL', NTERM_MRL, NTERM_READ, NDOFR                   &
                            , I_MRL, J_MRL, MRL)
      ENDIF
...
! Read MRR mass matrix
      ...
      IF (NTERM_MRR > 0) THEN                              ! Allocate and read KGG stiffness matrix
         ...
         WRITE(SC1,6092) LINKNO,MODNAM,HOUR,MINUTE,SEC,SFRAC
         CALL READ_MATRIX_1 ( LINK2N, L2N, OPND, CLOSE_IT, L2NSTAT, L2N_MSG, 'MRR', NTERM_MRR, NTERM_READ, NDOFR                   &
                            , I_MRR, J_MRR, MRR)
      ENDIF
...
! Deallocate arrays, but not PHIXA which may be needed in LINK5 to expand to G-set or IF_LTM needed in LINK9 for calc of QR_COL
      ...
      WRITE(SC1,12345) '       Deallocate EIGEN_VAL'   ;   CALL DEALLOCATE_EIGEN1_MAT ( 'EIGEN_VAL' )

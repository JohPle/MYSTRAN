...
!                                                              the ones on and above the diagonal (controlled by param SPARSTOR)
      ...
      INTEGER(LONG)                   :: NUM_KOO_DIAG_ZEROS  ! Number of zeros on the diag of KOO
...
! Decompose KOO, if there are any O-set DOF's
      ...
      IF (NDOFO > 0) THEN
         ...
         ELSE IF (SOLLIB == 'IntMKL') THEN                 ! Use Intel Math Kernel Library (MKL)
            ...
            WRITE(SC1,*) '     Intel MKL: initialize solver for KOO decomp'
            ...
            IF      (SPARSTOR == 'SYM   ') THEN
               ...
               MEM = 4.D0*(REAL(NDOFO+1)) + 12.D0*REAL(NTERM_KOO)
               WRITE(SC1,'(A,1ES9.2,A)') '                (Sparse arrays for KOO require ',mem/1000000.,' MB mem)'
               CALL SYM_MAT_DECOMP_IntMKL ( 'KOO', HANDLE, MKLFAC31, NDOFO, NTERM_KOO, I_KOO, J_KOO, KOO, DETERMINANT,             &
                                             CALC_INERTIA, INERTIA, IER_DECOMP )

            ELSE IF (SPARSTOR == 'NONSYM') THEN

               CALL SPARSE_MAT_DIAG_ZEROS ( 'KOO', NDOFO, NTERM_KOO, I_KOO, J_KOO, NUM_KOO_DIAG_ZEROS )
               NTERM_KOOs = (NTERM_KOO  + (NDOFO - NUM_KOO_DIAG_ZEROS))/2
               CALL ALLOCATE_SPARSE_MAT ( 'KOOs', NDOFO, NTERM_KOOs, SUBR_NAME )
               CALL CRS_NONSYM_TO_CRS_SYM ( 'KOO', NDOFO, NTERM_KOO, I_KOO, J_KOO, KOO, 'KOOs', NTERM_KOOs, I_KOOs, J_KOOs, KOOs )
               MEM = 4.D0*(REAL(NDOFO+1)) + 12.D0*REAL(NTERM_KOOS)
               WRITE(SC1,'(A,1ES9.2,A)') '                (Sparse arrays for KOO require ',mem/1000000.,' MB mem)'
               CALL SYM_MAT_DECOMP_IntMKL ( 'KOOs', HANDLE, MKLFAC31, NDOFO, NTERM_KOOs, I_KOOs, J_KOOs, KOOs, DETERMINANT,        &
                                             CALC_INERTIA, INERTIA, IER_DECOMP )
            ELSE
...
! Solve for GOA = -KOO(-1)*KAO'
      ...
      IF (NTERM_KAO > 0) THEN
         ...
         CALL SOLVE_GOA ( HANDLE )                         ! Solve for GOA matrix
         CLOSE_IT   = 'Y'
         CLOSE_STAT = 'KEEP'

...
!                                                            MYSTRAN DOF order: w1, th-x1, th-y1, w2, th-x2, th-y2, w3, th-x3, th-y3
      ...
      REAL(DOUBLE)                    :: XQL(5,2)          ! X, Y coords of the 5 quad points in quad local coords
...
! triangles will all connect. In local coords call these X5QL, Y5QL. First get coords of points 2-4 (pt 1 is origin)
      ...
      X5QL = X3QL*RAT
      Y5QL = Y3QL*RAT
      XQL(1,1) = ZERO   ;   XQL(1,2) = ZERO
      XQL(2,1) = X2QL   ;   XQL(2,2) = Y2QL
      XQL(3,1) = X3QL   ;   XQL(3,2) = Y3QL
      XQL(4,1) = X4QL   ;   XQL(4,2) = Y4QL
      XQL(5,1) = X5QL   ;   XQL(5,2) = Y5QL
...
trias:DO K=1,NUM_TRIAS
         ...
         TRIANGLE = K
         CALL ELMGM_TRIA ( TRIANGLE, XQL, V12, V13, L12, L13, ALPHA, THETA, X2TL, X3TL, Y3TL, TE_TRIA )
         IF (PROG_ERR > 0) THEN
            CALL OUTA_HERE ( 'Y' )
         ENDIF
         AREA_TRIA_(K) = X2TL*Y3TL/TWO

         CALL TPLT2 ( OPT_MIN4T, AREA_TRIA_(K), X2TL, X3TL, Y3TL, IERROR, KV_TT, PTV_TT, PPV_TT, B2V_TT, B3V_TT, S2V_TT, S3V_TT,   &
                     'N', DUM3 )
...
! ##################################################################################################################################
      ...
      SUBROUTINE ELMGM_TRIA ( TRIANGLE, XQL, V12, V13, L12, L13, ALPHA, THETA, X2TL, X3TL, Y3TL, TE_TRIA )
...
!                             | R2-tria |   | 0 -sin(THETA   cos(THETA) | | R2-quad |
      ...
      USE MODEL_STUF, ONLY            :  NUM_EMG_FATAL_ERRS, TE, XEL
      ...
      REAL(DOUBLE), INTENT(IN)        :: XQL(5,2)          ! X, Y coords of 5 quad nodes in quad local coords
...
!nnn  DLL_IMPORT MATADD
      ...
      REAL(DOUBLE)                    :: EPS1              ! Small number for comparing to near zero

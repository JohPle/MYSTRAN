...
!nnn  DLL_EXPORT TREL1
!nnn  DLL_IMPORT OURTIM
!nnn  DLL_IMPORT TMEM1
!nnn  DLL_IMPORT TPLT1
!nnn  DLL_IMPORT TPLT2
      ...
      REAL(DOUBLE)                    :: BBV(3,9)          ! Virgin strain-displ matrix for MIN3 in local TRIA coords
      ...
      IF ((OPT(2) == 'Y') .OR. (OPT(3) == 'Y') .OR. (OPT(4) == 'Y') .OR. (OPT(5) == 'Y')) THEN
         ...
         IF (TYPE(1:5) == 'TRIA3') THEN 
            IF (INTL_MID(1) /= 0) THEN
               CALL TMEM1 ( OPT, AREA, X2E, X3E, Y3E, BIG_BM )
            ENDIF
         ENDIF
         ...
         IF (TYPE == 'TRIA3   ') THEN
            IF (INTL_MID(2) /= 0) THEN
               CALL TPLT2 ( OPT, AREA, X2E, X3E, Y3E, IERROR, KV, PTV, PPV, B2V, B3V, S2V, S3V, 'Y', BIG_BB )
            ENDIF
         ENDIF

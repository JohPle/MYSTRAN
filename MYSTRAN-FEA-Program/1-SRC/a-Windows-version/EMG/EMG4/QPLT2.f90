...
!nnn  DLL_EXPORT QPLT2
!nnn  DLL_IMPORT OURTIM
!nnn  DLL_IMPORT MATMULT_FFF
!nnn  DLL_IMPORT ORDER_GAUSS
!nnn  DLL_IMPORT SHP2DQ
!nnn  DLL_IMPORT JAC2D
!nnn  DLL_IMPORT BBMIN4
!nnn  DLL_IMPORT MATMULT_FFF_T
!nnn  DLL_IMPORT MIN4SH
!nnn  DLL_IMPORT BSMIN4

      CHARACTER(LEN=LEN(BLNK_SUB_NAM)):: SUBR_NAME = 'QPLT2'
      CHARACTER(39*BYTE)              :: INT_ORDER         ! Character name of the integration order (used for debug output)
...
! Determine element thermal loads. 
      ...
      IF (OPT(2) == 'Y') THEN
         ...
         GAUSS_PT = 0
         INT_ORDER = 'for plate bending strains,    IORQ2B = '
         DO I=1,IORQ2B
            DO J=1,IORQ2B
               GAUSS_PT = GAUSS_PT + 1
               SSI  = SSS(I)
               SSJ  = SSS(J)
               CALL SHP2DQ ( I, J, NUM_NODES, SUBR_NAME, INT_ORDER, IORQ2B, SSI, SSJ, 'N', PSH, DPSHG )
...
! Bending stiffness terms:
         ...
         DO I=1,IORQ2B
            DO J=1,IORQ2B
               ...
               SSJ = SSS(J)
               GAUSS_PT = GAUSS_PT + 1
               CALL SHP2DQ ( I, J, NUM_NODES, SUBR_NAME, INT_ORDER, IORQ2B, SSI, SSJ, 'Y', PSH, DPSHG )
...
! If IORQ2T is neqative, a Jacobian weighted value for BS is used for all of the Gauss pts.
         ...
         IF (IORQ2T > 0) THEN
            ...
            GAUSS_PT = 0
            INT_ORDER = 'for transverse shear strains, IORQ2T = '
            DO I=1,IORDXX
               DO J=1,IORDXX
                  SSI = SSS(I)
                  SSJ = SSS(J)
                  GAUSS_PT = GAUSS_PT + 1
                  CALL SHP2DQ ( I, J, NUM_NODES, SUBR_NAME, INT_ORDER, IORQ2T, SSI, SSJ, 'Y', PSH, DPSHG )
...
! the SE2 matrices using FCONV, which was set in calling routine and is negative.
         ...
         SSJ = ZERO
         GAUSS_PT = 1
         CALL SHP2DQ ( 1, 1, NUM_NODES, SUBR_NAME, 'UNITY ', 1, SSI, SSJ, 'N', PSH, DPSHG )
         CALL JAC2D ( SSI, SSJ, XSD, YSD, 'N', JAC, JACI, DETJ )
         CALL MATMULT_FFF ( JACI, DPSHG, 2, 2, 4, DPSHX )
...
! Generate BE2, SE2 at Gauss points
         ...
         DO I=1,IORD_STRESS_Q4
            ...
            DO J=1,IORD_STRESS_Q4
               ...
               GAUSS_PT = GAUSS_PT + 1
               CALL SHP2DQ ( I, J, NUM_NODES, SUBR_NAME, ' ', IORD_STRESS_Q4, SSS(I), SSS(J), 'Y', PSH, DPSHG )
...
! Transverse shear terms. Calculate one set based on average at four Gauss points and 4 others at the Gauss points.
         ...
         DO I=1,2
            DO J=1,2
               ...
               SSJ = SSS(J)
               GAUSS_PT = GAUSS_PT + 1
               CALL SHP2DQ ( I, J, NUM_NODES, SUBR_NAME, 'TWO   ', 2, SSI, SSJ, 'N', PSH, DPSHG )
               CALL MIN4SH ( SSI, SSJ, XSD, YSD, 'N', NXSH, NYSH, DNXSHG, DNYSHG )
               CALL JAC2D ( SSI, SSJ, XSD, YSD, 'N', JAC, JACI, DETJ )
               CALL MATMULT_FFF ( JACI, DPSHG, 2, 2, 4, DPSHX )
               CALL MATMULT_FFF ( JACI, DNXSHG, 2, 2, 4, DNXSHX )  

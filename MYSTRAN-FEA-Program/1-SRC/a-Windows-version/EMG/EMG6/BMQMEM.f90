...
! Generates BM strain/displ matrix for 4 node membrane isoparametric element (quadratic)
...
! Calc outputs
      ...
      IF ((WRT_BUG_THIS_TIME == 'Y') .AND.(WRT_BUG(9) > 0)) THEN
         ...
         IF ((DABS(HBAR) > MXWARP) .AND. (DEBUG(4) ==  0)) THEN
            ...
            CALL BCHECK_2D ( BM_BMEANT, 'M', ID2, 3, 12, BW, 4 )
         ELSE
            CALL BCHECK_2D ( BM, 'M', ID1, 3, 8, BW, 4 )
         ENDIF

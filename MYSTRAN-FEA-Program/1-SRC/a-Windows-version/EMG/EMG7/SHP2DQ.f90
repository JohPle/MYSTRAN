...
! ELDATA output:
      ...
      IF ((WRT_BUG_THIS_TIME == 'Y') .AND.(WRT_BUG(7) > 0)) THEN
         ...
         IF      (IORZZZ > 0) THEN
            WRITE(BUG,1102) IGAUS, JGAUS, INT_ORDER, IORZZZ
         ELSE IF (IORZZZ < 0) THEN
            WRITE(BUG,1103) IGAUS, JGAUS, INT_ORDER, -IORZZZ
         ENDIF
...
! **********************************************************************************************************************************
 ...
 1102 FORMAT(51X,' Gauss point: I = ',I3,', J = ',I3,/,35X,'with integration order ',A,I2,/,                                       &
             35X,'and using Gaussian integration to determine strain-displ matrix'//)

 1103 FORMAT(51X,' Gauss point: I = ',I3,', J = ',I3,/,35X,'with integration order ',A,I2,/,                                       &
             33X,'and using Jacobian weighted average to determine strain-displ matrix'//)

 1112 format(51x,'Outputs fron subroutine SHP2DQ',/,51x,'------------------------------',/)

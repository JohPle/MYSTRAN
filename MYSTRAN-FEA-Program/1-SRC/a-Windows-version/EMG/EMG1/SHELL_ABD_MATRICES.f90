...
!   1) To get integrated effect of all plies in a shell element (QUAD4 or TRIA3) used in the development of the overall stiffness and
...
! Variables for composite elements
      ...
      INTEGER(LONG)                   :: LAM                ! Symm lam opt (for 1=SYM only plies on one side of elem C/L specified)
...
!                                                            NOTE: MATERIAL_PROPS_2D is called in procedure EMG for these
         ...
         ELSE
            DO I=1,3
               DO J=1,3
                  SHELL_A(I,J) = TM*EM(I,J)
                  SHELL_D(I,J) = IB*EB(I,J)
                  SHELL_B(I,J) = TM*TM*EBM(I,J)
               ENDDO
            ENDDO
         ENDIF
...
! Reset SHELL_T if null so that we can calc finite KS shear stiffness in subrs QPLT2, TPLT2. I did tests on setting SHELL_T to
! a factor of over 10^6 times bending stiffness and it works fine in the MIN4,T elements due to PHI_SQ

         IF ((DABS(SHELL_T(1,1)) < EPS1) .OR. (DABS(SHELL_T(2,2)) < EPS1)) THEN
            SHELL_T(1,1) = HALF*SHRFXFAC*( SHELL_D(1,1) + SHELL_D(2,2) )
         ...
         IF (WRT_BUG(1) > 0) THEN
            CALL BUG_SHELL_ABD_MATRICES ( 0, 13 )
         ENDIF
...
!                                                            NOTE: subr MATERIAL_PROPS_2D called here for these
         ...
         IF      (PCOMP(INTL_PID,3) == 0) THEN
            FAILURE_THEORY = 'NONE'
         ELSE IF (PCOMP(INTL_PID,3) == 1) THEN
            FAILURE_THEORY = 'HILL'
         ELSE IF (PCOMP(INTL_PID,3) == 2) THEN
            FAILURE_THEORY = 'HOFF'
         ELSE IF (PCOMP(INTL_PID,3) == 3) THEN
            FAILURE_THEORY = 'TSAI'
         ELSE IF (PCOMP(INTL_PID,3) == 4) THEN
            FAILURE_THEORY = 'STRE'
         ELSE IF (PCOMP(INTL_PID,3) == 5) THEN
            FAILURE_THEORY = 'STRN'
         ENDIF
...
!                                                            INTL_MID(3) will be reset to o
            ...
            IF      ((TYPE == 'QDMEM   ') .OR. (TYPE == 'QUAD4K  ') .OR. (TYPE == 'QUAD4   ') .OR.                                 &
               ...
               MASS_PER_UNIT_AREA = MASS_PER_UNIT_AREA + (RHO(1)*TPLY + NSM)
            ELSE IF ((TYPE == 'QDPLT1  ') .OR. (TYPE == 'QDPLT2  ') .OR.                                                           &
                     (TYPE == 'TRPLT1  ') .OR. (TYPE == 'TRPLT2  ')) THEN
               MASS_PER_UNIT_AREA = MASS_PER_UNIT_AREA + NSM
            ENDIF
...
! **********************************************************************************************************************************
 1946 FORMAT(' *ERROR  1946: PROGRAMMING ERROR IN SUBROUTINE ',A                                                                   &
                    ,/,14X,' ELEMENT TYPE ',A,' NOT HANDLED IN SUBR ',A)
...
! **********************************************************************************************************************************
      ...
      ELSE IF (WHAT == 20) THEN
         ...
         IF      (FAILURE_THEORY == 'NONE') THEN
            WRITE(BUG,92002) 'FT        = ', PCOMP(INTL_PID,3), '  no failure theory specified'
         ELSE IF (FAILURE_THEORY == 'HILL') THEN
            WRITE(BUG,92002) 'FT        = ', PCOMP(INTL_PID,3), '  use HILL failure theory'
         ELSE IF (FAILURE_THEORY == 'HOFF') THEN
            WRITE(BUG,92002) 'FT        = ', PCOMP(INTL_PID,3), '  use HOFF failure theory'
         ELSE IF (FAILURE_THEORY == 'TSAI') THEN
            WRITE(BUG,92002) 'FT        = ', PCOMP(INTL_PID,3), '  use TSAI failure theory'
         ELSE IF (FAILURE_THEORY == 'STRN') THEN
            WRITE(BUG,92002) 'FT        = ', PCOMP(INTL_PID,3), '  use STRN (max strain) failure theory'
         ELSE IF (FAILURE_THEORY == 'STRE') THEN
            WRITE(BUG,92002) 'FT        = ', PCOMP(INTL_PID,3), '  use STRE (max stress) failure theory'
         ENDIF

...
!                                                             comp 2 smallest then comp 3 then comp 1 then I3_OUT is 2, 3, 1 
      ...
      REAL(DOUBLE)                    :: VEC_SORT(3)         ! Sorted values of VX
...
! Initialize
      ...
      IF (TYPE == 'BUSH    ') THEN                         ! DO_IT = 'Y' when BUSH element has a defined CID in field 9 of CBUSH
         IF (BUSH_CID >= 0) THEN                           ! Otherwise, the TE matrix for BUSH is obtained from CID
            DO_IT = 'N'
         ENDIF
      ENDIF

...
!                                                            constr forces in subr WRITE_GRID_OUTPUTS for grids that have no force
...
! Process acceleration output requests for CB sol. 
      ...
      IF (WHAT == 'ACCE') THEN
         ...
         NUM  = 0
         DO I=1,NGRID                                      ! Prepare OGEL so subr WRITE_GRID_OUTPUTS can output the requested items
            IB = IAND(GROUT(I,INT_SC_NUM),IBIT(GROUT_ACCE_BIT))
            IF (IB > 0) THEN
               NUM = NUM + 1
               ...
               IF ((NUM == NREQ) .AND. (SC_OUT_REQ > 0)) THEN
                  CALL WRITE_GRID_OUTPUTS ( JVEC, NUM, WHAT, IHDR, ACCE_ALL_SAME_CID, WRITE_OGEL )
                  EXIT
               ENDIF
      ...
      ELSE IF (WHAT == 'DISP') THEN
         ...
         DO I=1,NGRID                                      ! Prepare OGEL so subr WRITE_GRID_OUTPUTS can output the requested items
            ...
            IF (IB > 0) THEN
               ...
               ENDDO
               IF ((NUM == NREQ) .AND. (SC_OUT_REQ > 0)) THEN
                  CALL WRITE_GRID_OUTPUTS ( JVEC, NUM, WHAT, IHDR, DISP_ALL_SAME_CID, WRITE_OGEL )
                  EXIT
               ENDIF
               IF ((SOL_NAME(1:12) == 'GEN CB MODEL') .AND. (JVEC == 1) .AND. (IROW_MAT >= 1)) THEN
                  DO J=1,OTMSKIP                           ! Write OTMSKIP blank separator lines
      ...
      ELSE IF (WHAT == 'OLOAD') THEN
         ...
         DO I=1,NGRID                                      ! Prepare OGEL so subr WRITE_GRID_OUTPUTS can output the requested items
            ...
            IF (IB > 0) THEN
               ...
               IF (NUM == NREQ) THEN
                  CALL WRITE_GRID_OUTPUTS ( JVEC, NUM, WHAT, IHDR, OLOAD_ALL_SAME_CID, WRITE_OGEL )
                  EXIT
               ENDIF
            ENDIF
         ENDDO

         IF ((POST /= 0) .AND. (ANY_OLOA_OUTPUT > 0)) THEN

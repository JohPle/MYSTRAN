...
!                                                            constr forces in subr WRITE_GRID_OUTPUTS for grids w/ no constr force
         ...
         DO I=1,NGRID
            ...
            IF ((IB1 > 0) .OR. (IB2 > 0) .OR. (SPCF_MEFM_MPF == 'Y')) THEN
               ...
               IF ((NUM == NREQ) .AND. (SC_OUT_REQ > 0)) THEN
                  CALL WRITE_GRID_OUTPUTS ( JVEC, NUM, WHAT, IHDR, SPCF_ALL_SAME_CID, WRITE_OGEL )
                  EXIT
               ENDIF
...
!                 ENDIF
               ...
               IF ((NUM == NREQ) .AND. (SC_OUT_REQ > 0)) THEN
                  CALL WRITE_GRID_OUTPUTS ( JVEC, NUM, WHAT, IHDR, MPCF_ALL_SAME_CID, WRITE_OGEL )
                  EXIT
               ENDIF

...
! Writes output for grid point related quantities (accels, displacements, eigenvectors, applied loads and SPC, MPC forces)
      ...
      USE WRITE_GRID_OUTPUTS_USE_IFs

      IMPLICIT NONE
 
      CHARACTER(LEN=LEN(BLNK_SUB_NAM)):: SUBR_NAME = 'WRITE_GRID_OUTPUTS'
         ...
         IF (DEBUG(200) > 0) THEN
            ...
            IF (WHAT == 'DISP') THEN
               ...
               IF    ((SOL_NAME(1:7) == 'STATICS') .OR. (SOL_NAME(1:8) == 'NLSTATIC')) THEN
                  WRITE(ANS,9322)
               ELSE IF ((SOL_NAME(1:8) == 'BUCKLING') .AND. (LOAD_ISTEP == 1)) THEN
                  WRITE(ANS,9322) SCNUM(JSUB)
               ELSE IF ((SOL_NAME(1:8) == 'BUCKLING') .AND. (LOAD_ISTEP == 2)) THEN
                  WRITE(ANS,9323) SCNUM(JSUB)
               ELSE IF (SOL_NAME(1:5) == 'MODES') THEN
                  WRITE(ANS,9323)
               ELSE IF (SOL_NAME(1:12) == 'GEN CB MODEL') THEN
                  WRITE(ANS,9324)

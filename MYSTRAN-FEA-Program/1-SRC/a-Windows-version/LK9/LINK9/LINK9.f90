...
! Main driver for calculating outputs requested in Case Control once the G-set unknowns have been solved for in prior LINK's
      ...
      USE IOUNT1, ONLY                :  ANSFIL, F06FIL, F25FIL, LINK1E, LINK1M, LINK1R, LINK2A, LINK2B, LINK2C, LINK2D, LINK2I,   &
                                         LINK2J, LINK2R, LINK2S, LINK5A, LINK5B, MOT4, MOU4, NEUFIL, OT4FIL, OU4FIL
      ...
      USE SCONTR, ONLY                :  BLNK_SUB_NAM, CC_ENTRY_LEN, COMM, IBIT, INT_SC_NUM, JTSUB, JTSUB_OLD, FATAL_ERR,          &
                                         FEMAP_VERSION,                                                                            &
                                         LINKNO, NDOFF, NDOFG, NDOFL, NDOFM, NDOFN, ndofo, NDOFR, NDOFS, NDOFSA, NGRID, NSUB, NVEC,&
      ...
      USE SCONTR, ONLY                :  GROUT_ACCE_BIT, GROUT_DISP_BIT, GROUT_OLOA_BIT, GROUT_SPCF_BIT, GROUT_MPCF_BIT,           &
                                         GROUT_GPFO_BIT, ELOUT_ELFN_BIT, ELOUT_ELFE_BIT, ELOUT_STRE_BIT, ELDT_F25_U_P_BIT
      ...
      CHARACTER( 1*BYTE)              :: F25OPN     = 'N'  ! 'Y'/'N' depending on whether file F25FIL is open
      ...
      CHARACTER( 1*BYTE)              :: L1EOPN    = 'N'   ! = 'Y' if file LINK1E is open
      CHARACTER( 1*BYTE)              :: L2DOPN    = 'N'   ! = 'Y' if file LINK1E is open
...
! or modal participation factors (but only do this if not a CB soln since MPFACTOR and MEFFMASS were calc'd in LINK6 for CB)
      ...
      IF ((SOL_NAME(1:5) == 'MODES') .OR. (SOL_NAME(1:12) == 'GEN CB MODEL')) THEN
         ...
         CALL ALLOCATE_EIGEN1_MAT ( 'EIGEN_VAL', NUM_EIGENS, 1, SUBR_NAME ) 
...
! CB OTM matrices (if CB soln) and open CB OTM output files (OU4(8) for grid related OTM's and OU4(9) for elem related OTM's)
      ...
      ELSE IF  (SOL_NAME(1:8) == 'BUCKLING') THEN
         ...
         NUM_SOLNS = 1                                     ! That is, we only run loops one time for each of the 2 calls to LINK9
...
! Process applied load output requests, if there are any and if this is STATICS.
...
! Process element force/stress output requests
         ...
         IF((SC_ELFE_OUTPUT > 0) .OR. (SC_ELFN_OUTPUT > 0) .OR. (SC_STRE_OUTPUT > 0) .OR. (ANY_U_P_OUTPUT > 0) .OR. (POST /= 0))THEN
            CALL OURTIM
...
! Rewind files containing G-set & S-set loads and read STIME so we can read loads file again for next subcase
         ...
         IF (LK9_PROC_NUM /= 1) THEN                       ! the sol'n so that we can calc the preloads for the differential stiff.
!zzzzz      CALL DEALLOCATE_COL_VEC ( 'UG_COL' )           ! These are re-calc'd for ALL elems as each elem KD is calculated
         ENDIF

...
!                                                            ! Option for stress output. Can be this or MAXS/SHEAR   
...
! possible outputs requested by the Case Control ELDATA command: 
 
!     CCELDT( 0) is for print to BUGFIL text file of elem geometric data
!     CCELDT( 1) is for print to BUGFIL text file of element ME
!     CCELDT( 2) is for print to BUGFIL text file of element PTE, PPE
!     CCELDT( 3) is for print to BUGFIL text file of element SE1,2 STE1,2
!     CCELDT( 4) is for print to BUGFIL text file of element KE
!     CCELDT( 5) is for print of UEL, PEL (all subcases)
!     CCELDT( 6) is for print outputs from subr's MIN4SH, SHP2D, SHP3D, JAC2D, JAC3D on elem shape fcns and Jacobian matrices
!     CCELDT( 7) is for print strain-displacement matrices in subroutines BMQMEM, BBDKQ, BHEX, BSMIN3, BSMIN4, BBMIN3, BBMIN4
!     CCELDT( 8) is for print check on element strain-displ matrices for rigid body motion and constant strain (subr BCHECK)
!     CCELDT( 9) is for write to F21FIL unformatted file of element ME
!     CCELDT(10) is for write to F22FIL unformatted file of element PTE, PPE
!     CCELDT(11) is for write to F23FIL unformatted file of element KE
!     CCELDT(12) is for write to F24FIL unformatted file of element SE1,2,3 STE1,2,3
!     CCELDT(13) is for write to F25FIL unformatted file of UEL, PEL (all subcases)
!     CCELDT(14) is currently not used
...
!           Bit  0 in OELDT indicates some requests for printed output after subr's ELMDAT, ELMGM1,2.
!           Bit  1 in OELDT indicates some requests for printed output of ME, PTE.
!           Bit  2 in OELDT indicates some requests for printed output of KE.
!           Bit  3 in OELDT indicates some requests for printed output of SE1,2,3 and STE1,2,3.
!           Bit  4 in OELDT indicates some requests for printed output of UEL, PEL (all S/C's).
!           Bit  5 in OELDT is currently not used 
!           Bit  6 in OELDT is currently not used
!           Bit  7 in OELDT is currently not used
!           Bit  8 in OELDT is currently not used
!           Bit  9 in OELDT indicates some requests for disk file output of ME, PTE.
!           Bit 10 in OELDT indicates some requests for disk file output of KE
!           Bit 11 in OELDT indicates some requests for disk file output of SE1,2,3 STE1,2,3
!           Bit 12 in OELDT indicates some requests for disk file output of UEL, PEL (all S/C's)
!           Bit 13 in OELDT is currently not used
!           Bit 14 in OELDT is currently not used
!           Bit 15 in OELDT is currently not used

!           Array OELDT is set in subr SCPRO based on the values read from CCELDT

!  ELDT: Array of 16 bits for each element to indicate whether any Case Control

!           ELDATA requests were made for that element
!           ELDT(j) tells whether internal element j has requests:
!           Bit  0 in ELDT is request for printed output after subr's ELMDAT, ELMGM1,2.
!           Bit  1 in ELDT is request for printed output of ME, PTE.
!           Bit  2 in ELDT is request for printed output of KE.
!           Bit  3 in ELDT is request for printed output of SE1,2,3 and STE1,2,3.
!           Bit  4 in ELDT is request for printed output of UEL, PEL (all S/C's).
!           Bit  5 in ELDT is currently not used 
!           Bit  6 in ELDT is currently not used
!           Bit  7 in ELDT is currently not used
!           Bit  8 in ELDT is currently not used
!           Bit  9 in ELDT is request for disk file output of ME, PTE.
!           Bit 10 in ELDT is request for disk file output of KE
!           Bit 11 in ELDT is request for disk file output of SE1,2,3 STE1,2,3
!           Bit 12 in ELDT is request for disk file output of UEL, PEL (all S/C's)
!           Bit 13 in ELDT is currently not used
!           Bit 14 in ELDT is currently not used
...
! Fixed size arrays
      ...
      INTEGER(LONG)                   :: ISOLID(MPSOLID)     = (/(0, I=1,MPSOLID)/)

...
! System control variables
      ...
      INTEGER(LONG), PARAMETER :: NCCCD                  = 30                ! Max number of words in array CC_CMD_DESCRIBERS
...
!                                                                              triangular factors (e.g.'KLL  ')
      ...
      CHARACTER(  1*BYTE)            :: PROTECTED        = 'N'               ! 'Y' if code is protected by Software Paaport
...
!                                                                              unlimited size problems 
      CHARACTER( 33*BYTE)            :: USERS_UNLOCK_KEY = ' '               ! Unlock key value input by user in the INI init file

      CHARACTER(  1*BYTE)            :: LINEAR_VALID_KEY = 'N'               ! 'Y' if user's key is valid for linear probs
      CHARACTER(  1*BYTE)            :: NONLIN_VALID_KEY = 'N'               ! 'Y' if user's key is valid for nonlinear probs
      CHARACTER(  1*BYTE)            :: MYSTRAN_VALID_KEY= 'N'               ! 'Y' if user's key is valid for all soln's in MYSTRAN

      INTEGER(LONG)            :: IBIT(0:15)          = (/(2**I, I=0,15)/)
      INTEGER(LONG)            :: JF(10)              = (/(   I, I=1,10)/)
      INTEGER(LONG)            :: UNLOCK_KEY_NUM      =   0      ! The num of the UNLOCK key input by user (or 0 if not a valid key)
      ...
      INTEGER(LONG)            :: JTSUB_OLD           =   0      ! Old (prior) value of JTSUB
...
!                                                                  the SPCADD set ID on the SPCADD card
      ...
      INTEGER(LONG)            :: MAX_STRESS_POINTS   =   0      ! Max number of pts for stress recovery within 1 element (for output
...
! named variables below. GROUT, ELOUT and ELDT all have 16 bits with the following ones used:
      ...
      INTEGER(LONG), PARAMETER :: ELDT_BUG_MASS_BIT   =   2      ! Bit pos in OELDT, ELDT for elem mass print requests
      INTEGER(LONG), PARAMETER :: ELDT_BUG_P_T_BIT    =   3      ! Bit pos in OELDT, ELDT for elem pres/therm load print requests
      ...
      INTEGER(LONG), PARAMETER :: ELDT_F21_MASS_BIT   =  10      ! Bit pos in OELDT, ELDT for elem mass file requests
      INTEGER(LONG), PARAMETER :: ELDT_F22_P_T_BIT    =  11      ! Bit pos in OELDT, ELDT for elem press/therm load file requests

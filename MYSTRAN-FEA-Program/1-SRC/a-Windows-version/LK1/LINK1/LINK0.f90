...
! Code here is used to test whether user has permission to run problems.
      ...
      IF (PROTECTED == 'N') THEN                           ! This test based on my old input key in MYSTRAN.INI
                                                           !    Make sure user has permission to run unlimited size probs
         IF ((UNLOCK /= 'Y') .AND. (LGRID > DEMO_GRID_LIMIT)) THEN
            WRITE(F06,1015) PROG_NAME, USERS_UNLOCK_KEY, PROG_NAME, PROG_NAME, DEMO_GRID_LIMIT
            CALL OUTA_HERE ( 'Y' )
         ENDIF

         IF (LGRID > DEMO_GRID_LIMIT) THEN                 !    See if user's key is valid for the SOL

            IF       ((SOL_NAME(1:7) == 'STATICS') .OR. (SOL_NAME(1:5) == 'MODES') .OR. (SOL_NAME(1:12) == 'GEN CB MODEL')) THEN

               IF (LINEAR_VALID_KEY /= 'Y') THEN
                  FATAL_ERR = FATAL_ERR + 1
                  WRITE(SC1, 9311) 'linear statics, modes, CB model generation', USERS_UNLOCK_KEY
                  WRITE(ERR, 9311) 'linear statics, modes, CB model generation', USERS_UNLOCK_KEY
                  WRITE(F06, 9311) 'linear statics, modes, CB model generation', USERS_UNLOCK_KEY
                  CALL OUTA_HERE ( 'Y' )
               ENDIF

            ELSE IF (SOL_NAME(1:8) == 'NLSTATIC') THEN

               IF (NONLIN_VALID_KEY /= 'Y') THEN
                  FATAL_ERR = FATAL_ERR + 1
                  WRITE(SC1, 9311) 'nonlinear statics', USERS_UNLOCK_KEY
                  WRITE(ERR, 9311) 'nonlinear statics', USERS_UNLOCK_KEY
                  WRITE(F06, 9311) 'nonlinear statics', USERS_UNLOCK_KEY
                  CALL OUTA_HERE ( 'Y' )
               ENDIF
            ...
            ELSE IF (SOL_NAME(1:8) == 'BUCKLING') THEN
               ...
               IF (NONLIN_VALID_KEY /= 'Y') THEN
                  FATAL_ERR = FATAL_ERR + 1
                  WRITE(SC1, 9311) 'linear buckling', USERS_UNLOCK_KEY
                  WRITE(ERR, 9311) 'linear buckling', USERS_UNLOCK_KEY
                  WRITE(F06, 9311) 'linear buckling', USERS_UNLOCK_KEY
                  CALL OUTA_HERE ( 'Y' )
               ENDIF
            ...
            ELSE
               ...
               FATAL_ERR = FATAL_ERR + 1
               WRITE(ERR,1826) SUBR_NAME, SOL_NAME
               WRITE(F06,1826) SUBR_NAME, SOL_NAME
...
! Make sure that there is at least 1 element
         ...
         IF ((PROTECTED == 'N') .AND. (DEBUG(178) > 0)) THEN
            WRITE(F06,1725) (VALID_MYSTRAN_KEYS(J),J=1,NUM_KEYS)
            WRITE(F06,1726) (VALID_LINEAR_KEYS(J) ,J=1,NUM_KEYS)
            WRITE(F06,1727) (VALID_NONLIN_KEYS(J) ,J=1,NUM_KEYS)
            WRITE(F06,*)                
            WRITE(F06,1728) UNLOCK_KEY_NUM, LINEAR_VALID_KEY, NONLIN_VALID_KEY
         ENDIF
...
res15:IF (RESTART == 'Y') THEN
         ...
         IF ( IAND(OELDT,IBIT(ELDT_BUG_SE_BIT)   ) > 0) THEN   ;   CHK_ELDT_BIT( 3) = 1   ;   ENDIF
         IF ( IAND(OELDT,IBIT(ELDT_BUG_KE_BIT)   ) > 0) THEN   ;   CHK_ELDT_BIT( 4) = 1   ;   ENDIF
         IF ( IAND(OELDT,IBIT(ELDT_BUG_SHPJ_BIT) ) > 0) THEN   ;   CHK_ELDT_BIT( 6) = 1   ;   ENDIF
         IF ( IAND(OELDT,IBIT(ELDT_BUG_BMAT_BIT) ) > 0) THEN   ;   CHK_ELDT_BIT( 7) = 1   ;   ENDIF
         IF ( IAND(OELDT,IBIT(ELDT_BUG_BCHK_BIT) ) > 0) THEN   ;   CHK_ELDT_BIT( 8) = 1   ;   ENDIF
         IF ( IAND(OELDT,IBIT(ELDT_F21_MASS_BIT) ) > 0) THEN   ;   CHK_ELDT_BIT( 9) = 1   ;   ENDIF
         IF ( IAND(OELDT,IBIT(ELDT_F22_P_T_BIT)  ) > 0) THEN   ;   CHK_ELDT_BIT(10) = 1   ;   ENDIF
         IF ( IAND(OELDT,IBIT(ELDT_F23_KE_BIT)   ) > 0) THEN   ;   CHK_ELDT_BIT(11) = 1   ;   ENDIF
         IF ( IAND(OELDT,IBIT(ELDT_F24_SE_BIT)   ) > 0) THEN   ;   CHK_ELDT_BIT(12) = 1   ;   ENDIF
...
res16:IF (RESTART == 'N') THEN
         ...
         DO I=1,NELE
            ...
            IF ((I1 > 0) .OR. (I2 > 0)) THEN
               ...
               WRITE(BUG,10)
               PLY_NUM = 0
               CALL EMG ( I   , OPT, 'N', SUBR_NAME )   ! Calc no matrices (all OPT = 'N')
...
! Temporary code fix: For statics problems cannot currently use IntMKL if there is an OMIT set. I need to fix code so that UO0
! can be solved for

         if ((sol_name(1:7) == 'STATICS') .and. (sollib == 'IntMKL') .and. (ndofo > 0)) then
            write(err,10001)
            write(f06,10001)
            sollib = 'ZZPACK'
         endif
   
10001 format(' *INFORMATION: Code not written yet for param SOLLIB = IntMKL for statics problems when there is an O-set',          &
                           ' (i.e. either ASET or OMIT Bulk Data entries).'                                                        &
                    ,/,14x,' Switching to PARAM, SOLLIB, ZZPACK')
...
! Grid point weight generator (model weight, c.g., etc.)
      ...
      I1 = IAND(OELDT,IBIT(ELDT_BUG_MASS_BIT))
      IF ((GRDPNT >= 0) .OR. (MEFFMASS_CALC == 'Y') .OR. (I1 > 0)) THEN
...
' *******************************************************************************************************************************',&
'****'                                                                                                                          ,/,&
' * The unlock key input (or omitted) in the ',A,' initialization file was invalid.',47x,'*'                                    ,/,&
' * That key is shown (between the quote signs here) as:',76X,'*'                                                               ,/,&
' *',129x,'*'                                                                                                                   ,/,&
' *',51X,'''',A,'''',43x,'*'                                                                                                   ,/,&
' *',129x,'*'                                                                                                                   ,/,&
' * If you purchased the unlimited size version of MYSTRAN, make sure this is the key sent to you.',30x,'*'                     ,/,&
' * It must have no blanks and must contain all of the 33 characters assigned to you. ',46x,'*'                                 ,/,&
' * It is put into the ',A,' initialization (INI) file as the line: ',62x,'*'                                                   ,/,&
' *',129x,'*'                                                                                                                   ,/,&
' *',43x,' UNLOCK  nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn',44x,'*'                                                                   ,/,&
' *',129x,'*'                                                                                                                   ,/,&
' * where UNLOCK must begin in col 1 and the n','''','s are the 33 characters of your unlock key and must begin in col 9.'         &
   ,18x,'*'  ,/,&
' * If you do not have a valid key, this version of ',A,' is limited to ',I3,' grid points.',42x,'*'                            ,/,&
' * To obtain a version that has no MYSTRAN limitation on the number of grid points, contact the author via email at:',15x,'*'  ,/,&
' *',129x,'*'                                                                                                                   ,/,&
' *',54X,'DrBillC@earthlink.net',54x,'*'                                                                                        ,/,&
' *',129x,'*'                                                                                                                   ,/,&
' *******************************************************************************************************************************',&
'****')
 ...
 1725 FORMAT(' Valid key numbers for all of MYSTRAN are:', 99i3)

 1726 FORMAT(' Valid key numbers for statics only are  :', 99i3)

 1727 FORMAT(' Valid key numbers for modes   only are  :', 99i3)

 1728 FORMAT(' The number of the UNLOCK key input by the user was: ',i3,/,                                                         &
             '                      Is this key valid for linear : ',2x,a,/,                                                       &
             '                      Is this key valid for nonlin : ',2x,a,/)
 ...
 1826 FORMAT(' *ERROR  1826: PROGRAMMING ERROR IN SUBROUTINE ',A                                                                   &
                    ,/,14X,' SOL_NAME = "',A,'" HAS NOT BEEN PROGRAMMED FOR VALID KEY YET')

 1827 FORMAT(' *ERROR  1827: THERE MUST BE AT LEAST 1 GRID IN THE BULK DATA. HOWEVER, NGRID = ',I8)

 9311 FORMAT(' Your UNLOCK key, listed below, is not valid for ',A,' problems',/,1X,'"',A,'"')
...
! ##################################################################################################################################
      ...
      USE SCONTR, ONLY                :  ELDT_BUG_DAT1_BIT, ELDT_BUG_DAT1_BIT, ELDT_BUG_MASS_BIT, ELDT_BUG_P_T_BIT ,               &
                                         ELDT_BUG_SE_BIT  , ELDT_BUG_KE_BIT  , ELDT_BUG_SHPJ_BIT, ELDT_BUG_BMAT_BIT,               &
                                         ELDT_BUG_BCHK_BIT, ELDT_F21_MASS_BIT, ELDT_F22_P_T_BIT , ELDT_F23_KE_BIT  ,               &
...
! **********************************************************************************************************************************
      ...
      DO II=1,NELE
         ...
         IF ( IAND(ELDT(II),IBIT(ELDT_BUG_MASS_BIT) ) > 0) THEN   ;   I00 = 1   ;   WRT_BUG(2) = 1   ;   ENDIF
         IF ( IAND(ELDT(II),IBIT(ELDT_BUG_P_T_BIT)  ) > 0) THEN   ;   I00 = 1   ;   WRT_BUG(3) = 1   ;   ENDIF
         IF ( IAND(ELDT(II),IBIT(ELDT_BUG_SE_BIT)   ) > 0) THEN   ;   I00 = 1   ;   WRT_BUG(4) = 1   ;   ENDIF
         IF ( IAND(ELDT(II),IBIT(ELDT_BUG_KE_BIT)   ) > 0) THEN   ;   I00 = 1   ;   WRT_BUG(5) = 1   ;   ENDIF
         ...
         IF ( IAND(ELDT(II),IBIT(ELDT_F21_MASS_BIT) ) > 0) THEN   ;   I00 = 1   ;   WRT_FIJ(1) = 1   ;   ENDIF
         IF ( IAND(ELDT(II),IBIT(ELDT_F22_P_T_BIT)  ) > 0) THEN   ;   I00 = 1   ;   WRT_FIJ(2) = 1   ;   ENDIF
         ...
         IF (I00 > 0) THEN
            ...
            IF ((WRT_FIJ(1) > 0) .OR. (WRT_FIJ(2) > 0) .OR. (WRT_FIJ(3) > 0) .OR. (WRT_FIJ(4) > 0)) THEN
               CALL WRITE_EOFIL ( 0 )
            ENDIF

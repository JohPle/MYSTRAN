...
!xx   ELSE IF (JCARD(2)(1:8) == 'F06COL  ') THEN
!xx      PARNAM = 'F06_COL_START'
!xx      CALL I4FLD ( JCARD(3), JF(3), I4PARM )
!xx      IF (IERRFL(3) == 'N') THEN
!xx         LOWER = 2
!xx         UPPER = 99999999
!xx         IF ((I4PARM >= LOWER) .AND. (I4PARM <= UPPER)) THEN
!xx            F06_COL_START = I4PARM
!xx         ELSE
!xx            WARN_ERR = WARN_ERR + 1
!xx            WRITE(ERR,101) CARD
!xx            WRITE(ERR,1196) PARNAM,LOWER,UPPER,JCARD(3),F06_COL_START
!xx            IF (SUPWARN == 'N') THEN
!xx               IF (ECHO == 'NONE  ') THEN
!xx                  WRITE(F06,101) CARD
!xx               ENDIF
!xx               WRITE(F06,1196) PARNAM,LOWER,UPPER,JCARD(3),F06_COL_START
!xx            ENDIF
!xx         ENDIF
!xx      ENDIF

!xx      CALL BD_IMBEDDED_BLANK   ( JCARD,0,3,0,0,0,0,0,0 )! Make sure that there are no imbedded blanks in field 3
!xx      CALL CARD_FLDS_NOT_BLANK ( JCARD,0,0,4,5,6,7,8,9 )! Issue warning if fields 4-9 not blank
!xx      CALL CRDERR ( CARD )                              ! CRDERR prints errors found when reading fields
...
! MKLFACij tell what form (positive def, indefinite, etc) to use in subr SYM_MAT_DECOML_IntMKL

      ELSE IF (JCARD(2)(1:6) == 'MKLFAC') THEN
         PARNAM = 'MKLFACij'
         CALL CHAR_FLD ( JCARD(3), JF(3), CHRPARM )
         IF (IERRFL(3) == 'N') THEN
            CALL LEFT_ADJ_BDFLD ( CHRPARM )
            IF      (JCARD(2)(7:8) == '21') THEN
               MKLFAC21 = CHRPARM
            ELSE IF (JCARD(2)(7:8) == '31') THEN
               MKLFAC31 = CHRPARM
            ELSE IF (JCARD(2)(7:8) == '41') THEN
               MKLFAC41 = CHRPARM
            ELSE IF (JCARD(2)(7:8) == '42') THEN
               MKLFAC42 = CHRPARM
            ELSE IF (JCARD(2)(7:8) == '61') THEN
               MKLFAC61 = CHRPARM
            ELSE IF (JCARD(2)(7:8) == '62') THEN
               MKLFAC62 = CHRPARM
            ELSE IF (JCARD(2)(7:8) == '63') THEN
               MKLFAC63 = CHRPARM
            ELSE
               WARN_ERR = WARN_ERR + 1
               WRITE(ERR,101) CARD
...
! MKLSTATS tells whether to write stats on decomp of matrix in SYM_MAT_DECOMP_IntMKL

      ELSE IF (JCARD(2)(1:8) == 'MKLSTATS') THEN
         PARNAM = 'MKLSTATS  '
         CALL CHAR_FLD ( JCARD(3), JF(3), CHRPARM )
         IF (IERRFL(3) == 'N') THEN
            CALL LEFT_ADJ_BDFLD ( CHRPARM )
            ...
            ELSE
               WARN_ERR = WARN_ERR + 1
               WRITE(ERR,101) CARD
               WRITE(ERR,1189) PARNAM, 'Y OR N', CHRPARM, MKLSTATS
               IF (SUPWARN == 'N') THEN
                  IF (ECHO == 'NONE  ') THEN
                     WRITE(F06,101) CARD
                  ENDIF
                  WRITE(F06,1189) PARNAM, 'Y OR N', CHRPARM, MKLSTATS
               ENDIF
            ENDIF
         ENDIF
...
! MKLMATST tells whether to use MKL_DSS_SYMMETRIC or MKL_DSS_NON_SYMMETRIC in the Intel MKL function DSS_DEFINE_STRUCTURE

      ELSE IF (JCARD(2)(1:8) == 'MKLMATST') THEN
         PARNAM = 'MKLMATST'
         CALL CHAR_FLD ( JCARD(3), JF(3), CHRPARM )
         IF (IERRFL(3) == 'N') THEN
            CALL LEFT_ADJ_BDFLD ( CHRPARM )
            IF      (CHRPARM(1:6) == 'SYM   ') THEN
               MKLMATST = 'SYM   '
            ELSE IF (CHRPARM(1:6) == 'NONSYM') THEN
               WARN_ERR = WARN_ERR + 1
               WRITE(ERR,1197) PARNAM
               WRITE(F06,1197) PARNAM
               MKLMATST = 'NONSYM'
            ELSE
               WARN_ERR = WARN_ERR + 1
               WRITE(ERR,101) CARD
               WRITE(ERR,1189) PARNAM, 'Y OR N', CHRPARM, MKLMATST
               IF (SUPWARN == 'N') THEN
                  IF (ECHO == 'NONE  ') THEN
                     WRITE(F06,101) CARD
                  ENDIF
                  WRITE(F06,1189) PARNAM, 'Y OR N', CHRPARM, MKLMATST
               ENDIF
            ENDIF
         ENDIF
...
! SOLLIB sets the method for solving equations (LANCZOS, etc)
      ...
      ELSE IF (JCARD(2)(1:8) == 'SOLLIB  ') THEN
         ...
         IF (IERRFL(3) == 'N') THEN
            CALL LEFT_ADJ_BDFLD ( CHRPARM )
            IF      (CHRPARM == 'IntMKL  ') THEN
               SOLLIB = 'IntMKL  '
            ELSE IF (CHRPARM == 'ZZPACK  ') THEN
               SOLLIB = 'ZZPACK  '
            ELSE
               WARN_ERR = WARN_ERR + 1
               WRITE(ERR,101) CARD
               WRITE(ERR,1189) PARNAM,'ZZPACK, or IntMKL',CHRPARM,SOLLIB
               IF (SUPWARN == 'N') THEN
                  IF (ECHO == 'NONE  ') THEN
                     WRITE(F06,101) CARD
                  ENDIF
                  WRITE(F06,1189) PARNAM,'ZZPACK, or IntMKL',CHRPARM,SOLLIB
               ENDIF
            ENDIF
         ENDIF

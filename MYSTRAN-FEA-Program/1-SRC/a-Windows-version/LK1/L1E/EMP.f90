...
elems:DO I=1,NELE
         ...
         I1 = IAND(ELDT(I),IBIT(ELDT_F21_MASS_BIT))
         IF (I1 > 0) THEN
            WRT_FIJ(1) = 1
         ENDIF
...
! Prints out info on the formulation of stiffness arrays for subr ESP, which generates the arrays
      ...
      CHARACTER(1*BYTE), INTENT(IN)   :: WHAT              ! Indicator of where this subr was called from in subr ESP0_2
      ...
      INTEGER(LONG)    , INTENT(IN)   :: MGG_COL           ! Row number of MGG matrix where KE(J,K) goes 
      INTEGER(LONG)    , INTENT(IN)   :: MGG_ROW           ! Col number of MGG matrix where KE(J,K) goes 

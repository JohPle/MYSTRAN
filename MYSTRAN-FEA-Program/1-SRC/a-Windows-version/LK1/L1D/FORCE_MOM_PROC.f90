...
!   (1) Fron local (as defined on the FORCE/MOMENT card) to basic
...
!                                                            so CYCLE & read next record
            ...
            CALL GEN_T0L ( GRID_ID_ROW_NUM, ICID, THETAD, PHID, T12 )! Get coord transformation matrix - local to basic

            CALL MATMULT_FFF ( T12, F1, 3, 3, 1, F2 )      ! Transform coords from local (on FORCE/MOMENT card) to basic
                                                           ! F2 has the force/moment values in basic coords
         ELSE                                              ! Coord system is basic, so equate F2, F1
            DO J=1,3
               F2(J) = F1(J)
            ENDDO 
         ENDIF
...
j_do14:        DO J=1,NCORD
               ...
               CALL GEN_T0L ( GRID_ID_ROW_NUM, ICID, THETAD, PHID, T12 )! Get coord transformation matrix - basic to global

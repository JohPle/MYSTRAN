...
!                                                            Lanczos basis constructed by ARPACK subr dsaupd
      ...
      INTEGER(LONG)                   :: LJUTERM           ! Max no. terms allowed in JU. This has to be estimated. If the U matrix
!                                                            turns out to be full, LJUTERM could be as large as N*(N-1)/2

      INTEGER(LONG)                   :: LUTERM            ! Value for NUTERN to allocate memory to array Uy
      INTEGER(LONG)                   :: NUTERM            ! Number of terms in array U.  This is calculated in subr SFAC
      INTEGER(LONG)                   :: NJUTERM           ! Number of terms in JU(I). This is calculated in subr SFAC
! **********************************************************************************************************************************
...
! **********************************************************************************************************************************
      ...
      ELSE IF (SOLLIB == 'IntMKL') THEN
         ...
         IF (EIG_FRQ2 > EPS1) THEN
            IF (EIG_FRQ1 > EPS1) THEN
               CALL EST_NUM_EIGENS_IntMKL ( EIG_FRQ1, NUM_NEG_TERMS1 )
            ENDIF
            CALL EST_NUM_EIGENS_IntMKL ( EIG_FRQ2, NUM_NEG_TERMS2 )
            IF (NUM_NEG_TERMS2 > 0) THEN
               NUM_EST_EIGENS = NUM_NEG_TERMS2 - NUM_NEG_TERMS1
            ELSE
               ! ?
...
! Write RFAC, if requested
      ...
      IF (DEBUG(40) == 1) THEN
         IF      (EIG_LAP_MAT_TYPE(1:3) == 'DPB') THEN
            CALL WRITE_MATRIX_BY_ROWS ( 'MATRIX [KLL - sigma*KLLD] IN LAPACK BAND FORM', RFAC, LDRFAC, NDOFL )
         ELSE
            CALL WRITE_MATRIX_BY_ROWS ( 'MATRIX [KLL - sigma*MLL ] IN LAPACK BAND FORM', RFAC, LDRFAC, NDOFL )
         ENDIF
      ENDIF

! Dellocate arrays for I_KLL, J_KLL, KLL.
...
! ##################################################################################################################################
      ...
      SUBROUTINE EST_NUM_EIGENS_IntMKL ( FREQ, NUM_NEG_TERMS )

      USE SCONTR, ONLY                :  NTERM_KLLs, NTERM_MLLs, NTERM_KMSMs
      USE SPARSE_MATRICES, ONLY       :  I_KLLs, J_KLLs, KLLs, I_MLLs, J_MLLs, MLLs, I_KMSMs, J_KMSMs, KMSMs, SYM_KLLs, SYM_MLLs
      ...
      INTEGER(LONG), INTENT(OUT)      :: NUM_NEG_TERMS     ! Number of negative terms on the diagonal of RFAC
      INTEGER(LONG)                   :: IER_DECOMP        ! Overall error indicator from MKL_SOLVER
      INTEGER(LONG)                   :: NUM_KLL_DIAG_ZEROS! Number of zeros on the diag of KLL
      INTEGER(LONG)                   :: NUM_MLL_DIAG_ZEROS! Number of zeros on the diag of MLL

      REAL(DOUBLE) , INTENT(IN)       :: FREQ              ! 
      REAL(DOUBLE)                    :: SIGMA             ! 
      REAL(DOUBLE)                    :: INERTIA(3)        ! Triplet of numbers that define the inertia of the KMSM matrix

! **********************************************************************************************************************************
      NUM_NEG_TERMS = 0
      SIGMA = (TWO*PI*FREQ)**2

! Set up KMSM one way or another depending on whether SPARSTOR is SYM or NONSYM and call MKL_SOLVER


      IF (SPARSTOR == 'SYM   ') THEN
                                                           ! Add KLL minus SIGMA*MLL
         CALL MATADD_SSS_NTERM ( NDOFL, 'KLL', NTERM_KLL,  I_KLL,  J_KLL, SYM_KLL, '-SIGMA*MLL', NTERM_MLL,                        &
                                 I_MLL, J_MLL, SYM_MLL, 'KMSM', NTERM_KMSM )
         CALL ALLOCATE_SPARSE_MAT ( 'KMSM', NDOFL, NTERM_KMSM, SUBR_NAME )
         CALL MATADD_SSS       ( NDOFL, 'KLL',  NTERM_KLL,  I_KLL,  J_KLL,  KLL,  ONE, '-SIGMA*MLL', NTERM_MLL,                    &
                              I_MLL, J_MLL, MLL, -SIGMA, 'KMSM', NTERM_KMSM, I_KMSM, J_KMSM, KMSM )

         CALL MKL_SOLVER ( INERTIA, IER_DECOMP )
         WRITE(SC1, * )                                 ! Advance 1 line for the screen messages         
         WRITE(SC1,12345) '       Deallocate KMSM  '   ;   CALL DEALLOCATE_SPARSE_MAT ( 'KMSM' )

      ELSE
                                                           ! Put KLL and MLL in symmetric format
         ...
         NTERM_MLLs = (NTERM_MLL  + (NDOFL - NUM_MLL_DIAG_ZEROS))/2
         CALL ALLOCATE_SPARSE_MAT ( 'MLLs', NDOFL, NTERM_MLLs, SUBR_NAME )
         CALL CRS_NONSYM_TO_CRS_SYM ( 'MLL', NDOFL, NTERM_MLL, I_MLL, J_MLL, MLL, 'MLLs', NTERM_MLLs, I_MLLs, J_MLLs, MLLs )

                                                           ! Add KLLs minus SIGMA*MLLs
         CALL MATADD_SSS_NTERM ( NDOFL, 'KLLs', NTERM_KLLs,  I_KLLs,  J_KLLs, SYM_KLLs, '-SIGMA*MLLs', NTERM_MLLs,                 &
                                 I_MLLs, J_MLLs, SYM_MLLs, 'KMSMs', NTERM_KMSMs )
         CALL ALLOCATE_SPARSE_MAT ( 'KMSMs', NDOFL, NTERM_KMSMs, SUBR_NAME )
         CALL MATADD_SSS       ( NDOFL, 'KLLs',  NTERM_KLLs,  I_KLLs,  J_KLLs,  KLLs,  ONE, '-SIGMA*MLLs', NTERM_MLLs,             &
                                 I_MLLs, J_MLLs, MLLs, -SIGMA, 'KMSMs', NTERM_KMSMs, I_KMSMs, J_KMSMs, KMSMs )

         CALL MKL_SOLVER ( INERTIA, IER_DECOMP )
         CALL DEALLOCATE_SPARSE_MAT ( 'KLLs' )
         WRITE(SC1, * )                                 ! Advance 1 line for the screen messages         
         WRITE(SC1,12345) '       Deallocate KMSMs '   ;   CALL DEALLOCATE_SPARSE_MAT ( 'KMSMs' )

      ENDIF

      NUM_NEG_TERMS = INERTIA(2)
...
! **********************************************************************************************************************************
      ...
      END SUBROUTINE EST_NUM_EIGENS_IntMKL

! ##################################################################################################################################
! Routine that uses the Intel Math Kernel Library (MKL) to factor a matrix

      SUBROUTINE MKL_SOLVER ( INERTIA, IER_DECOMP )

      USE PENTIUM_II_KIND
      USE SCONTR, ONLY                :  NTERM_KMSMs
      USE PARAMS, ONLY                :  MKLFAC42
      USE SPARSE_MATRICES, ONLY       :  I_KMSMs, J_KMSMs, KMSMs
      USE MKL_DSS
      ...
      INTEGER(LONG)                   :: IntMKL_IER

      REAL(DOUBLE) , INTENT(OUT)      :: INERTIA(3)
      REAL(DOUBLE)                    :: DETERMINANT

!***********************************************************************************************************************************
! Initialize the solver

      WRITE(SC1,*) '     Intel MKL: initialize solver'
      WRITE(SC1,*) '     Calling MKL dss_create'
...
! Factor the matrix
      ...
      CALC_INERTIA = 'Y'                                   ! Needed for number of eigens estimate

      CALL SYM_MAT_DECOMP_IntMKL ( 'KMSMs', HANDLE, MKLFAC42, NDOFL, NTERM_KMSMs, I_KMSMs, J_KMSMs, KMSMs, DETERMINANT,            &
                                    CALC_INERTIA, INERTIA, IER_DECOMP )
! Deallocate solver storage

      WRITE(SC1,*) '     Intel MKL: delete data structures created for the MKL'
      ...
      IF (IER_DECOMP /= 0) THEN
         FATAL_ERR = FATAL_ERR + 1
         WRITE(SC1,*) '     IntMKL_IER = ', IntMKL_ier,' occurred in MKL routine: ',DSS_NAME
         WRITE(ERR,959) SUBR_NAME, 'KMSMs', IER_DECOMP, DSS_NAME
         WRITE(F06,959) SUBR_NAME, 'KMSMs', IER_DECOMP, DSS_NAME
         CALL OUTA_HERE ( 'Y' )
      ENDIF

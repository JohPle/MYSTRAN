...
!                                                            file in called subr SYM_MAT_DECOMP_LAPACK (ABAND = band form of KLL)
      ...
      INTEGER(LONG)                   :: NUM_KMSM_DIAG_ZEROS! Number of zeros on the diag of KMSM
...
! If this is not a CB soln, dellocate arrays for KLL. ! Keep arrays for I_MLL, J_MLL, MLL (or I_KLLD, J_KLLD, KLLD) - needed later
! to calc gen mass
...
! Factor KMSM
      ...
      ELSE IF (SOLLIB == 'IntMKL') THEN
         ...
         WRITE(SC1,*) '     Intel MKL: initialize solver for KMSM decomp'
         ...
         IF      (SPARSTOR == 'SYM   ') THEN
            ...
            MEM = 4.D0*(REAL(NDOFL+1)) + 12.D0*REAL(NTERM_KMSM)
            WRITE(SC1,'(A,1ES9.2,A)') '                (Sparse arrays for KMSM require ',mem/1000000.,' MB mem)'
            CALL SYM_MAT_DECOMP_IntMKL ( 'KMSM', HANDLE, MKLFAC41, NDOFL, NTERM_KMSM, I_KMSM, J_KMSM, KMSM, DETERMINANT,           &
                                          CALC_INERTIA, INERTIA, IER_DECOMP )

         ELSE IF (SPARSTOR == 'NONSYM') THEN

            CALL SPARSE_MAT_DIAG_ZEROS ( 'KMSM', NDOFL, NTERM_KMSM, I_KMSM, J_KMSM, NUM_KMSM_DIAG_ZEROS )
            NTERM_KMSMs = (NTERM_KMSM  + (NDOFL - NUM_KMSM_DIAG_ZEROS))/2
            CALL ALLOCATE_SPARSE_MAT ( 'KMSMs', NDOFL, NTERM_KMSMs, SUBR_NAME )
            CALL CRS_NONSYM_TO_CRS_SYM ( 'KMSM' , NDOFL, NTERM_KMSM , I_KMSM, J_KMSM, KMSM,                                        &
                                         'KMSMs',        NTERM_KMSMs, I_KMSMs, J_KMSMs, KMSMs )
            MEM = 4.D0*(REAL(NDOFL+1)) + 12.D0*REAL(NTERM_KMSMs)
            WRITE(SC1,'(A,1ES9.2,A)') '                (Sparse arrays for KMSM require ',mem/1000000.,' MB mem)'
            CALL SYM_MAT_DECOMP_IntMKL ( 'KMSMs', HANDLE, MKLFAC41, NDOFL, NTERM_KMSMs, I_KMSMs, J_KMSMs, KMSMs, DETERMINANT,      &
                                          CALC_INERTIA, INERTIA, IER_DECOMP )
...
! [KLL - sigma*MLL]*Vec = alpha*MLL*Vec (EIGEN_VAL = sigma + 1/alpha)
         ...
         ELSE IF (SOLLIB == 'IntMKL') THEN
            CALL FBS_IntMKL ( HANDLE, NDOFL, MVEC )
         ELSE
            FATAL_ERR = FATAL_ERR + 1
            WRITE(ERR,9991) SUBR_NAME, SOLLIB
            WRITE(F06,9991) SUBR_NAME, SOLLIB

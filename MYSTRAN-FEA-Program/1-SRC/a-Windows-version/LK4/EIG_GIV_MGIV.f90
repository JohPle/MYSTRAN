...
! Put stiffness and mass matrices in form required by LAPACK band matrix and write them out, if requested.
      ...
      IF ((DEBUG(40) == 1) .OR. (DEBUG(40) == 3)) THEN
         CALL WRITE_MATRIX_BY_ROWS ( 'STIFFNESS MATRIX KLL IN LAPACK BAND FORM', ABAND, LDAB, NDOFL )
      ENDIF
      ...
      IF (SOL_NAME(1:8) == 'BUCKLING') THEN
         ...
         IF ((DEBUG(40) == 2) .OR. (DEBUG(40) == 3)) THEN
            CALL WRITE_MATRIX_BY_ROWS ( 'DIFF STIFF MATRIX KLLD IN LAPACK BAND FORM'     , BBAND, LDBB, NDOFL )
         ENDIF
      ELSE
         ...
         IF ((DEBUG(40) == 2) .OR. (DEBUG(40) == 3)) THEN
            CALL WRITE_MATRIX_BY_ROWS ( 'MASS MATRIX MLL IN LAPACK BAND FORM'     , BBAND, LDBB, NDOFL )
         ENDIF
      ENDIF

! If this is not a CB soln, dellocate arrays for KLL.      ! Keep arrays for MLL, KLLD. Need them later to calc gen mass

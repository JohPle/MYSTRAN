...
! is called herein, will run faster. MATMULT_SFF is called in each "Reverse commumication loop" in DSBAND for the LANCZOS method.
      ...
      IF      (SPARSTOR == 'SYM   ') THEN
         ...
         IF (SOL_NAME(1:8) == 'BUCKLING') THEN
            ...
            WRITE(SC1,4092) LINKNO,MODNAM,HOUR,MINUTE,SEC,SFRAC
            CALL CRS_SYM_TO_CRS_NONSYM ( 'KLLD', NDOFL, NTERM_KLLD, I_KLLD, J_KLLD, KLLD, 'KLLDn', NTERM_KLLDn, I_KLLDn, J_KLLDn, KLLDn, 'Y' )
...
! **********************************************************************************************************************************
! Solve eigenvalue problem (NOTE: SOLLIB was switched to ZZPACK in LINK0 if it had been IntMKL since code for eigenvalues via the 
! Intel MKL has not been coded. A message was written in LINK0 if this switch is made)

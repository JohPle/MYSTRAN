...
!     Use the G-set displacements from LINK5 to solve for the outputs requested in Case Control.
      ...
      USE IOUNT1, ONLY                :  F06FIL, IN0, IN1, IN0_MSG, IN1_MSG, IN0FIL, INFILE, LEN_INPUT_FNAME, LEN_RESTART_FNAME,   &
                                         ...
                                         ANS, ANSFIL, NEU, NEUFIL, NEUSTAT, RESTART_FILNAM,                                        &
      ...
      USE SCONTR, ONLY                :  COMM, FATAL_ERR, LINKNO_START, LSETLN, LSETS, LSUB, NDOFL, NSETS, NSUB, NTSUB,            &
                                         PROG_NAME, PROTECTED, PRINTENV, PROTECTED, RESTART, SETLEN, SOL_NAME, WARN_ERR
      ...
      INTEGER(LONG)                   :: IEND               ! Col where a character string stops having non blanks 
      ...
      REAL(DOUBLE) , ALLOCATABLE      :: UG_NORM(:,:)       ! Norm of ght UG_COL vector
...
! Get SP environment variables and reset PROTECTED if necessary. NOTE: PROTECTED = 'Y' is default in module SCONTR
      ...
      CALL GET_SP_ENVIRON_VARS
      IF ((ALLOWABLE_PROTECTED_SOLNS(1:5) == 'TRIAL') .OR. (ALLOWABLE_PROTECTED_SOLNS(1:12) == 'ALL FEATURES')) THEN
         PROTECTED = 'Y'
      ELSE
         PROTECTED = 'N'
      ENDIF
...
! Write logo and copyright notice to screen. Then write MYSTRAN start time/date
      ...
      IF (PROTECTED == 'Y') THEN
         CALL GET_CHAR_STRING_END ( ALLOWABLE_PROTECTED_SOLNS, IEND )
         WRITE(SC1,116) PROG_NAME, MYSTRAN_VER_NUM, MYSTRAN_VER_MONTH, MYSTRAN_VER_DAY, MYSTRAN_VER_YEAR, MYSTRAN_VER_AUTH,        &
                        ALLOWABLE_PROTECTED_SOLNS(1:IEND)
      ELSE
         WRITE(SC1,117) PROG_NAME, MYSTRAN_VER_NUM, MYSTRAN_VER_MONTH, MYSTRAN_VER_DAY, MYSTRAN_VER_YEAR, MYSTRAN_VER_AUTH
      ENDIF
...
! LINK was beginning and then warm restarted in the next LINK if the INI file is modified to have LINK_START = that LINK number
      ...
      IF (LINKNO_START == 0) THEN
         ...
         IF (ALLOWABLE_PROTECTED_SOLNS == 'TRIAL') THEN    ! Print SP environ vars, if requested (but always print them if 'TRIAL')
            CALL PRINT_SP_ENVIRON_VARS
         ELSE IF (PRINTENV == 'Y') THEN
            CALL PRINT_SP_ENVIRON_VARS
         ENDIF
...
! **********************************************************************************************************************************
  ...
  116 FORMAT(/,1X,A,' Version',5(1X,A),/,' (this ',A,' edition is SP protected)')

  117 FORMAT(/,1X,A,' Version',5(1X,A),/,' (this edition is not SP protected)')
...
!zzzzz      ELSE
!zzzzz         PERCENT_CHANGE(LOAD_ISTEP,NL_ITER_NUM) = ZERO
!zzzzz      ENDIF   
!zzzzz   ENDIF
      ENDIF

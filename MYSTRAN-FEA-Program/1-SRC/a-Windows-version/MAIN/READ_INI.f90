...
! INI variable WRT LOG is set to a high number (like 99)
...
!                                                            At end, ECHO is reset by ECHO = PERM_ECHO
      ...
      CHARACTER( 1*BYTE)              :: UNLOCK_LINE_FOUND ! 'Y', 'N' indicator if INI file has an UNLOCK line (only 1 allowed)
...
! Set default for USERS_UNLOCK_KEY
...
! Give message and CYCLE if error occurs. EXIT if encounter EOF/EOR. Otherwise read CARD.
            ...
            ELSE IF ((CARD(1:1) /= '$') .AND. (CARD(1:) /= ' ')) THEN
               ...
               IF (CARD(1:8) == 'UNLOCK  ') THEN
                  IF (UNLOCK_LINE_FOUND == 'N') THEN
                     USERS_UNLOCK_KEY = CARD(9:42)
                     UNLOCK_LINE_FOUND = 'Y'
                     CALL KEY_CHECK
                  ELSE
                     WRITE(SC1,1514)
                     CALL WRITE_FILNAM ( INIFIL, SC1, 15 )
                     WRITE(SC1, * )
                     WRITE(SC1, * ) 'Processing terminated'
                     STOP
                  ENDIF

               ELSE IF (CARD(1:8) ==  'DEF DIR ') THEN
                  IBEGIN = 9
...
!xx            ELSE IF (CARD(1:8) == 'PROTCTD') THEN
!xx               CALL C8FLD0 ( JCARD_08(2), JF(2), CHAR8)
!xx               PROTECTED = CHAR8(1:1)
!xx               CALL CARD_FLDS_NOT_BLANK0 ( JCARD_08, 0,3,4,5,6,7,8,9, WRT_HDR, WRT_CARD )
!xx               CALL CRDERR0 ( CARD, FLD_ERR_MSG )
...
! **********************************************************************************************************************************
 ...
 1514 FORMAT(' *FATAL ERROR: Only 1 "UNLOCK" entry allowed in the initialization file:')
...
! ##################################################################################################################################
      ...
      USE PENTIUM_II_KIND, ONLY       :  BYTE, LONG, DOUBLE
      USE KEY_VALUES, ONLY            :  KEYS_THAT_UNLOCK, NUM_KEYS, VALID_LINEAR_KEYS, VALID_NONLIN_KEYS, VALID_MYSTRAN_KEYS
      USE SCONTR, ONLY                :  UNLOCK, UNLOCK_KEY_NUM , LINEAR_VALID_KEY, NONLIN_VALID_KEY, MYSTRAN_VALID_KEY
...
! **********************************************************************************************************************************
      ...
      UNLOCK_KEY_NUM = 0
      DO II=1,NUM_KEYS
         IF ( USERS_UNLOCK_KEY == KEYS_THAT_UNLOCK(II) ) THEN
            UNLOCK = 'Y'
            UNLOCK_KEY_NUM = II
            EXIT
         ENDIF
      ...
      IF (UNLOCK_KEY_NUM > 0) THEN

         MYSTRAN_VALID_KEY = 'N'
         DO II=1,NUM_KEYS
            IF ( UNLOCK_KEY_NUM == VALID_MYSTRAN_KEYS(II) ) THEN
               MYSTRAN_VALID_KEY = 'Y'
               EXIT
            ENDIF
         ENDDO

         IF (MYSTRAN_VALID_KEY == 'Y') THEN
         ...
         ELSE
            ...
            LINEAR_VALID_KEY = 'N'
            DO II=1,NUM_KEYS
               IF ( UNLOCK_KEY_NUM == VALID_LINEAR_KEYS(II) ) THEN
                  LINEAR_VALID_KEY = 'Y'
                  EXIT
               ENDIF
            ENDDO
            ...
            NONLIN_VALID_KEY = 'N'
            DO II=1,NUM_KEYS
               IF ( UNLOCK_KEY_NUM == VALID_NONLIN_KEYS(II) ) THEN
                  NONLIN_VALID_KEY = 'Y'
                  EXIT
               ENDIF
            ENDDO
